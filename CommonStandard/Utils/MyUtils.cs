...
using System.Linq;
using ProtoBuf;
using Instrument = Common.EntityModels.Instrument;
...
namespace Common.Utils
{
    public static class MyUtils
    {
        ...
        /// </summary>
        public static string GetFuturesContractSymbol(string baseSymbol, int month, int year) => $"{baseSymbol}{GetFuturesMonthSymbol(month)}{year % 10}";
        ...
        {
            TimeSpan elapseDateTimeCloseime = value - Epoch;
            return (long)elapseDateTimeCloseime.TotalSeconds;
        }
        ...
        public static T ProtoBufDeserialize<T>(MemoryStream ms) => Serializer.Deserialize<T>(ms);
        ...
        /// <summary>
        /// Returns the number of business days between two dates, not including the final day.
        /// </summary>
        ...
        public static void SetSessionTimes(IEnumerable<OHLCBar> bars, Instrument instrument)
        {
            ...
            {
                int dotw = bar.DateTimeClose.DayOfWeek.ToInt();
                ...
                if (openingSessions.ContainsKey(dotw))
                {
                    if ((int)openingSessions[dotw].OpeningDay != dotw)
                    {
                        ...
                        bar.DateTimeOpen = new DateTime(bar.DateTimeClose.Year, bar.DateTimeClose.Month, bar.DateTimeClose.Day).AddDays(-daysToMoveBack) + openingSessions[dotw].OpeningTime;
                    }
                    else
                    {
                        bar.DateTimeOpen = new DateTime(bar.DateTimeClose.Year, bar.DateTimeClose.Month, bar.DateTimeClose.Day) + openingSessions[dotw].OpeningTime;
                    }
                }
                else
                {
                    bar.DateTimeOpen = new DateTime(bar.DateTimeClose.Year, bar.DateTimeClose.Month, bar.DateTimeClose.Day, 0, 0, 0);
                }

                //set closing time
                if (closingSessions.ContainsKey(dotw))
                {
                    bar.DateTimeClose = new DateTime(bar.DateTimeClose.Year, bar.DateTimeClose.Month, bar.DateTimeClose.Day) + closingSessions[dotw];
                }
                else
                {
                    bar.DateTimeClose = new DateTime(bar.DateTimeClose.Year, bar.DateTimeClose.Month, bar.DateTimeClose.Day, 23, 59, 59);
                }
            }
        }

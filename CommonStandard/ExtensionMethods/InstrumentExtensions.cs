...
namespace Common.ExtensionMethods
{
    public static class InstrumentExtensions
    {
        ...
        public static string TagsAsString(this Instrument instrument)
        {

            var tags = instrument.Tags;
            return (tags == null || tags.Count == 0) ? "" : string.Join(", ", tags.Select(x => x.Name));

        }

        public static TimeZoneInfo GetTimeZoneInfo(this Instrument instrument)
        {
            if (instrument.Exchange != null)
            {
                return TimeZoneInfo.FindSystemTimeZoneById(
                    string.IsNullOrEmpty(instrument.Exchange.Timezone)
                        ? "UTC"
